...
data_path=pathlib.Path('drive/MyDrive/cv/Original Data/train')
...
cnn.add(Dense(units=4,activation='softmax'))    # 4 classes: scab? rot? rust? healty?

cnn.compile(loss='sparse_categorical_crossentropy',optimizer=Adam(learning_rate=0.000001),metrics=['accuracy'])
hist=cnn.fit(train_ds,epochs=10,validation_data=test_ds,verbose=2)

print('정확률=',cnn.evaluate(test_ds,verbose=0)[1]*100)

cnn.save('ald_201921786.h5')	# 미세 조정된 모델을 파일에 저장

import pickle
f=open('ald_201921786.txt','wb')
