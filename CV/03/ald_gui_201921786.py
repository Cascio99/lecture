...
cnn=tf.keras.models.load_model('ald_201921786.h5')	# 모델 읽기
dog_species=pickle.load(open('ald_201921786.txt','rb'))	# 상태 인식
         
class AppleLeafDiseaseRecognition(QMainWindow):
    def __init__(self) :
        super().__init__()
        self.setWindowTitle('사과 나뭇잎 상태 인식')
        self.setGeometry(200,200,700,100)
       
        fileButton=QPushButton('나뭇잎 사진 열기',self) 
        recognitionButton=QPushButton('상태 인식',self)
    ...
    def pictureOpenFunction(self):
        fname=QFileDialog.getOpenFileName(self,'나뭇잎 사진 읽기','./')           
        self.img=cv.imread(fname[0])
        if self.img is None: sys.exit('파일을 찾을 수 없습니다.')  
        
        cv.imshow('Apple leaf image',self.img)          
...
win=AppleLeafDiseaseRecognition() 
win.show()
app.exec_()